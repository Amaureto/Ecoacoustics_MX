...
```{r}
ggplot(RanasPTC, aes(x = Especie, y = Longitud_media_mm, size = Peso_promedio_gr)) +geom_point(aes(colour=Color_dominante))+labs(x="Especies de Anuros", y="Longitud media (mm)", size="Peso promedio (gr)")+ylim(0,16)+ theme_classic()+theme(axis.text.x = element_text(angle=90, 20, vjust = 0.5, hjust=0, size = 9))+ geom_text(aes(label = No_individuos, size = 18, hjust=0.5, vjust=-2, position="stack"))+ scale_colour_manual(name="Color_dominante", values=c('Verde oscuro'='chartreuse4', 'Café'='tan4','Café oscuro'='orange4', 'Café medio'='tan3','Verde claro'='olivedrab2','Gris'='wheat3','Verde limón'='lawngreen','Café claro'='lightgoldenrod4','Verde'='green3','Dorado'='gold3')) +guides(colour=FALSE)
```
